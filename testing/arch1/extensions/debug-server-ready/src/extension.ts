...
// TODO: Re-enable when terminal data listening is implemented
...
class ServerReadyDetector extends vscode.Disposable {
	...
	static rememberShellPid(_session: vscode.DebugSession, _pid: number) {
		// TODO: Re-enable when terminal data listening is implemented
		// const detector = ServerReadyDetector.detectors.get(session);
		// if (detector) {
	...
	static async startListeningTerminalData() {
		if (!this.terminalDataListener) {
			// TODO: Fix when onDidWriteTerminalData is available in VS Code API
			// this.terminalDataListener = vscode.window.onDidWriteTerminalData(async (e: any) => {
