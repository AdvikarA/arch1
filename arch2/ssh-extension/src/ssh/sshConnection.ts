...
import { Client, ClientChannel, ClientErrorExtensions, ExecOptions, ShellOptions, SSHConnectConfig } from 'ssh2';
import { Server } from 'net';
import { SocksClient } from 'socks';

export interface LocalSSHConnectConfig extends SSHConnectConfig {
...
const defaultOptions: Partial<LocalSSHConnectConfig> = {
    reconnect: false,
    port: 22,
    reconnectTries: 3,
    reconnectDelay: 5000
...
export default class SSHConnection extends EventEmitter {
    public config: LocalSSHConnectConfig;
     ...
     */
    connect(c?: LocalSSHConnectConfig): Promise<SSHConnection> {
    ...
    addTunnel(SSHTunnelConfig: SSHTunnelConfig): Promise<SSHTunnelConfig & { server: Server }> {
        ...
        if (this.getTunnel(SSHTunnelConfig.name)) {
            ...
            return new Promise((resolve, reject) => {
                ...
                // SOCKS functionality temporarily disabled
                if (SSHTunnelConfig.socks) {
                    throw new Error('SOCKS tunneling is not currently supported');
                } else {
